...
/**
 * Service Worker로 메시지를 보내고 응답을 Observable로 받음
 */
export const sendMessage = <T>(message: unknown): Observable<Result<Error, T>> =>
  from(
...
export const requestTabCapture = (
  tabId: number
): Observable<Result<Error, TabCaptureResponse>> => {
 ...
 * 현재 활성 탭 ID 가져오기
 */
export const getCurrentTabId = (): Observable<Result<Error, number>> =>
  from(
